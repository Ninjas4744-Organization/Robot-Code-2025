...
//            Transform2d transform2 = RobotState.getInstance().getTransform(target);
            ...
            if(RobotState.getInstance().getReefLevel() < 4){
                a = 2.5;
                b = 2;
            }
...
//            double yVel = _yp.calculate(-transform.getY());
        ...
        return Commands.sequence(
            Commands.run(() -> {
                ...
                Logger.recordOutput("Other/Reef Target", _currentReefTarget);
...
//            Logger.recordOutput("0 dist", angle);
    ...
    public double distToTarget(){
        Logger.recordOutput("Other/Dist", RobotState.getInstance().getDistance(FieldConstants.getClosestReefTarget(
                RobotState.getInstance().isReefRight(),
                _outtakeExtraMove)));
